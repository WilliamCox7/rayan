# Claude Todo

## Update History

- The shattering needs some work. I don't like the rewrote physics aspect. The cultural agents
  are not capable of rewriting the physics engine but they can exploit it. Lets come up with a
  physics exploit that would exterminate a entire population of people almost immediately.
  I'm thinking that entire culture is destroyed and the agent in charge ends up becoming the
  commissioner since they have no affect on any characters anymore (except a handful-they become
  a lost race).
- Its also important to note that agents can't do whatever they want _with their characters_. They are bound
  to simulate free will. Each person they control has a context they must abide by (very deterministic). In
  order for a character to learn magic, the agent would have manufacture a situation where the character learns
  the magic. Like writing a book of spells and leaving it for them to find and read.

## Update Magic

- I think I want items attached to magic. I think the agents agree
  to make magic items for their people that must be held in order for an AI to use an exploit.
  The holder must also say the correct words and the AI must perform the exploit paired to that word.
  This would restrict an agent from using whatever exploit they want and it gives an illusion of control
  to the holder. They also agreed on banned exploits because of their extreme destructive nature
  (like the one used in the shattering). The agents agreed that if an agent disobeys these rules,
  there will be permanant consequences to their culture (not sure what those might be yet).

## New Element

- I kind of like the idea of introducing a periodic "element"
